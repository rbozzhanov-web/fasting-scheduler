...
self.addEventListener("fetch",e=>{
  ...
  /* Кэш отвечает первым, а не как запасной вариант на случай, если сеть не
     ответила: у приложения нет своего сервера и всё, что оно грузит по сети —
     это тот же статический shell, что уже лежит в caches.open(C). Слабый
     сигнал, wifi с порталом или его отсутствие — обычный случай для того,
     кто открывает это в самолёте или ангаре, а не запасной. fetch() не имеет
     собственного таймаута — раньше это значило ждать сеть столько, сколько
     ей вздумается, на КАЖДЫЙ файл приложения, прежде чем вообще увидеть кэш.
     Теперь сеть всё ещё идёт параллельно и обновляет кэш — но для следующего
     запуска, не блокируя этот. */
  e.respondWith(
    caches.match(e.request).then(hit=>{
      const revalidate=fetch(e.request).then(r=>{
      ...
      return hit||revalidate.then(r=>r||caches.match(e.request));
    })
  );
});
